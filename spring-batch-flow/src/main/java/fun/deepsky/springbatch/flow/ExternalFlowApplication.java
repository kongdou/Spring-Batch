...
public class ExternalFlowApplication {

	public static void main(String[] args) {
		ApplicationContext context = new ClassPathXmlApplicationContext("job-external-flow.xml");

		JobLauncher launcher = (JobLauncher) context.getBean("jobLauncher");

		Job job = (Job) context.getBean("externalFlowJob");
		...
		try {
			launcher.run(job, new JobParametersBuilder().addDate("date", new Date())
					...
					.addString("writeTarget", "file:target/flow/outputFile_10.csv").toJobParameters());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
